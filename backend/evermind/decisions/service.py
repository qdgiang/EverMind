...
class DecisionsService:
    ...
    def _decision_payload(self, decision: Decision, **extra: Any) -> dict:
        payload = {
            ...
            "effect_window": (
                ...
                if decision.effect_window_from
                else None
            ),
    ...
    def _sort_key(self, ts: datetime, recorded_at: datetime, stable_event_id: str | None) -> tuple:
    ...
    def _standing_occupants(self, units: list[str], prefixes: list[str]) -> dict[str, Decision]:
        ...
        *,
        ...
        decision = Decision(
            ...
            approved_by_role_at_act=(
                self.org.rank_of(approved_by) if approved_by is not None else None
            ),
            created_from=cmd.created_from,
            confidence=(
                1.0
                if cmd.created_from in (CreatedFrom.MARKER, CreatedFrom.DASHBOARD)
                else cmd.confidence
            ),
        ...
        if cmd.delegation_message_id is not None:
            self.session.add(
                DecisionCitation(
                    decision_id=decision.id,
                    message_id=cmd.delegation_message_id,
    ...
    def _effective_write(self, winner: Decision, plan: UnitPlan, command: CommandEnvelope) -> dict:
        ...
        for row in list(self.session.scalars(select(EffectiveUnit))):
            prefix_hit = any(
                row.unit_key == p or row.unit_key.startswith(p + "|")
    ...
    def _propose(self, cmd: ProposeDecision) -> dict:
        ...
        if plan.has_new_task:
            ...
            for op in cmd.ops:
                ...
                rewritten.append(OpSpec(target=target, facet=op.facet, op=op.op, value=op.value))
            cmd = cmd.model_copy(
                update={
                    "ops": rewritten,
                    "scope_target": (
                        f"task:{new_task_id}"
                        if cmd.scope_target == "NEW_TASK"
                        else cmd.scope_target
                    ),
                }
            )
            plan = derive_unit_plan(cmd.ops)
        ...
        if plan.occupied_units and all(
            ...
            for decision in {d.id: d for d in occupants.values()}.values():
                for citation in cmd.citations:
                    self._add_citation_once(decision.id, citation, CitationKind.CORROBORATION)
                ...
                self._emit(
                    ...
                    decision.id,
                    {
                        ...
                        "restated_by": cmd.decided_by_user_id,
                        "at": _iso(ts),
                    },
                    ts,
                    cmd,
        ...
        conf_ok = cmd.created_from in (CreatedFrom.MARKER, CreatedFrom.DASHBOARD) or (
            cmd.confidence is not None and cmd.confidence >= self.tau
        )
        ...
        if cmd.force_proposed:
            ...
            for unit, occupant in occupants.items():
                if not self.authority.rank_gate_ok(
                    cmd.decided_by_user_id, occupant.decided_by_role_at_time
                ):
                    hold_reason = "rank_gate"
                    hold_extra = {
                        "unit": unit,
                        "standing_decision_id": occupant.id,
                        "standing_maker": occupant.decided_by_user_id,
                    }
                    break
                if (
                    actor_rank == occupant.decided_by_role_at_time
                    and occupant.decided_by_user_id != cmd.decided_by_user_id
                    and not self.authority.comparable(
                        cmd.decided_by_user_id, occupant.decided_by_user_id
                    )
                ):
                    ...
                    hold_extra = {
                        "unit": unit,
                        "standing_decision_id": occupant.id,
                        "peers": [cmd.decided_by_user_id, occupant.decided_by_user_id],
                    }
                    break
        ...
        if hold_reason is None:
            ...
            if newer:
                ...
                decision = self._insert_decision(
                    ...
                    approval_via=(
                        ApprovalVia.DELEGATION if cmd.delegated_by_user_id is not None else None
                    ),
                    approved_by=cmd.delegated_by_user_id,
                )
                decision.superseded_by_decision_id = standing.id
                self._emit(
                    "decision_born_superseded",
                    "decision",
                    decision.id,
        ...
        merged_into = self._find_pending_twin(plan) if cmd.effect_window is None else None
        ...
        if cmd.force_proposed:
            project_id = self._context_project(cmd)
            if project_id is None and cmd.scope_target.startswith("task:"):
                task = self.task_port.get_task_view(int(cmd.scope_target.split(":", 1)[1]))
                project_id = task.project_id if task else None
            approvers = self.org.leads_of_project(project_id) if project_id is not None else []
            approvers.extend(user.id for user in self.org.list_personas() if user.role_rank == 3)
    ...
    def _authorize_ops(self, actor_id: int, cmd: ProposeDecision, targets: list[str] | None = None):
        ...
        for target in targets:
            results.append(
                self.authority.can_decide_target(actor_id, cmd.scope, target, cmd.context_group_id)
            )
    ...
    def _version_conflict(
        self, expected: str, plan: UnitPlan, occupants: dict[str, Decision]
    ) -> dict | None:
        ...
        return {"status": "version_conflict", "expected_version": expected, "diff": diff}
    ...
    def _overlapping_window_for(
        ...
        for decision, unit_key in rows:
            if decision.id == exclude_id or unit_key not in units:
                continue
            if decision.effect_window_from <= until and start <= decision.effect_window_until:
                return decision
        return None
    ...
    def _find_pending_twin(self, plan: UnitPlan) -> Decision | None:
        ...
        for candidate in candidates:
            ...
            if (
                set(other.occupied_units) == set(plan.occupied_units)
                and other.unit_values == plan.unit_values
            ):
                return candidate
        return None

    def _withdraw_own_older(self, new: Decision, plan: UnitPlan, cmd: ProposeDecision) -> list[int]:
        withdrawn: list[int] = []
        rows = self.session.execute(
            select(Decision, DecisionUnit.unit_key)
            .join(DecisionUnit, DecisionUnit.decision_id == Decision.id)
    ...
    def _fallback_approvers(
        self, cmd: ProposeDecision, occupants: dict[str, Decision], hold_reason: str | None
    ) -> list[int]:
    ...
    def _add_citation_once(
        self, decision_id: int, citation: CitationSpec, kind: CitationKind
    ) -> None:
    ...
    def _approve(self, cmd: ApproveProposal) -> dict:
        ...
        if cmd.source_message_id is not None:
            self._add_citation_once(
                decision.id,
                CitationSpec(
                    message_id=cmd.source_message_id,
        ...
        task_ctx = (
            {"new_task_id": decision.new_task_id, "project_id": decision.context_project_id}
            if decision.new_task_id is not None
            else {}
        )
        ...
        if decision.effect_window_from is not None:
            self._emit(
                ...
                decision.id,
                self._decision_payload(
                    decision, windowed=True, approved_by=approver, via=via.value, **task_ctx
                ),
                decision.ts,
                cmd,
            )
        ...
        self._emit(
            ...
            decision.id,
            self._decision_payload(
                decision, approved_by=approver, via=via.value, **task_ctx, **write
            ),
            decision.ts,
            cmd,
        )
    ...
    def _authorize_decision_targets(self, actor_id: int, decision: Decision):
        ...
        results = [
            self.authority.can_decide_target(actor_id, decision.scope, t, None)
    ...
    def _revalidation_issues(
        self, decision: Decision, plan: UnitPlan, occupants: dict[str, Decision]
    ) -> list[dict]:
        issues: list[dict] = []
            ...
            for task_id in plan.task_ids:
                ...
                if view.status is TaskStatus.CANCELED:
                    canceling = self.session.scalar(
                        select(EffectiveUnit.decision_id).where(
                            EffectiveUnit.unit_key == f"task:{task_id}|status"
                        )
                    )
                    issues.append(
                        {
                            "kind": "target_canceled",
                            "task_id": task_id,
        ...
        if decision.effect_window_from is not None:
            ...
            overlap = self._overlapping_window_for(
                decision.effect_window_from,
                decision.effect_window_until,
                plan.occupied_units,
                exclude_id=decision.id,
            )
            if overlap is not None:
                issues.append({"kind": "window_overlap", "overlaps_decision_id": overlap.id})
        return issues
    ...
    def _reject(self, cmd: RejectProposal) -> dict:
        ...
        if decision.status is DecisionStatus.PROPOSED:
            allowed = (
                is_maker or rank_ok or self._authorize_decision_targets(actor, decision).allowed
            )
            if not allowed:
                self._emit(
                    "challenge_filed",
            ...
            reason = RejectedReason.VETO if cmd.reason == "veto" else RejectedReason.DISMISSED
            decision.status = DecisionStatus.REJECTED
            ...
            self._emit(
                ...
                "decision",
                decision.id,
                {"decision_id": decision.id, "rejected_reason": reason.value, "by": actor},
                _utcnow(),
                cmd,
            )
            return {"status": "rejected", "decision_id": decision.id, "reason": reason.value}
        ...
        if decision.status is DecisionStatus.EFFECTIVE:
            ...
            for row in list(
                self.session.scalars(
                    select(EffectiveUnit).where(EffectiveUnit.decision_id == decision.id)
                )
            ):
                self.session.delete(row)
            self.session.flush()
            ...
            self._emit(
                ...
                {
                    "decision_id": decision.id,
                    "rejected_reason": "veto",
                    "by": actor,
                    "resurrected": resurrected,
                    "retraction": True,
                },  # feed retraction entries (G20)
                _utcnow(),
                cmd,
            )
            return {
    ...
    def _resurrect_predecessors(self, rejected: Decision, cmd: CommandEnvelope) -> list[int]:
        ...
        for predecessor in predecessors:
            ...
            vacant = all(
                self.session.scalar(select(EffectiveUnit).where(EffectiveUnit.unit_key == unit))
                is None
                for unit in units
            )
            if not vacant:
            ...
            if predecessor.effect_window_from is None:
                for unit in units:
                    self.session.add(EffectiveUnit(unit_key=unit, decision_id=predecessor.id))
            resurrected.append(predecessor.id)
            self._emit(
                "decision_resurrected",
                "decision",
                predecessor.id,
                self._decision_payload(predecessor, resurrected_by_rejection_of=rejected.id),
                _utcnow(),
                cmd,
            )
        self.session.flush()
    ...
    def _bulk(self, cmd: BulkProposalAction) -> dict:
        pendings = list(
            self.session.scalars(select(Decision).where(Decision.status == DecisionStatus.PROPOSED))
        )
        acted: list[int] = []
        skipped: list[dict] = []
        for decision in pendings:
            ...
            if cmd.action == "approve_all":
                ...
                self._emit(
                    ...
                    {
                        "decision_id": decision.id,
                        "rejected_reason": "dismissed",
                        "by": cmd.actor_user_id,
                        "bulk": cmd.action,
                    },
                    _utcnow(),
                    cmd,
                ...
                acted.append(decision.id)
        return {"status": "bulk_done", "action": cmd.action, "acted": acted, "skipped": skipped}
    ...
    def _record_signal(self, cmd: RecordSignal) -> dict:
        ...
        self._emit(
            ...
            cmd.task_id or 0,
            {
                ...
                "normalized_topic": cmd.normalized_topic,
                "excerpt": cmd.excerpt,
                "message_id": cmd.source_message_id,
                "ts": _iso(ts),
                "window_id": cmd.window_id,
    ...
    def _append_corroboration(self, cmd: AppendCorroboration) -> dict:
        ...
        self._emit(
            ...
            "decision",
            decision.id,
            {"decision_id": decision.id, "message_ids": [cmd.citation.message_id]},
            _as_utc(cmd.ts) or _utcnow(),
            cmd,
        )

...
class DecisionsService:
    ...
    def _propose(self, cmd: ProposeDecision) -> dict:
        ...
        if (cmd.review_reason == "signal_promotion"
            ...
            return {"status": "invalid",
                    "error": "signal promotion requires blocked status and waiting metadata"}
        ...
        hold_extra: dict = {}
        if cmd.review_reason == "signal_promotion":
            # Signal promotions are always reviewed by the owning task team.
        ...
        if cmd.review_reason == "signal_promotion":
            ...
            approvers.extend(user.id for user in self.org.list_personas()
                             if user.role_rank == 3)
            approvers = list(dict.fromkeys(approvers))
        else:
            approvers = auth.approvers or self._fallback_approvers(
                cmd, occupants, hold_reason)
    ...
    def _valid_signal_promotion_ops(ops: list[OpSpec]) -> bool:
        ...
        if len(ops) != 2:
            return False
        status = [op for op in ops if op.facet == "status"
                  and op.op == "set" and op.value == "blocked"]
        waiting = [op for op in ops if op.facet == "blocked_waiting_on"
                   and op.op == "set" and isinstance(op.value, dict)]
        return (len(status) == len(waiting) == 1
                and status[0].target == waiting[0].target
                and status[0].target.startswith("task:"))
    ...
    def _signal_promotion_approvers(self, scope_target: str) -> list[int]:
        ...
        coordinators = [u.id for u in self.org.list_personas() if u.role_rank == 3]
        ...
        return resolved or coordinators
    ...
    def _approve(self, cmd: ApproveProposal) -> dict:
        ...
        if (decision.review_reason == "signal_promotion"
                and not self._valid_signal_promotion_ops(ops)):
            return {"status": "invalid", "error": "invalid signal promotion",
                    "decision_id": decision.id}
        ...
        if decision.review_reason == "signal_promotion":
            ...
            if approver not in approvers:
                return {"status": "forbidden", "decision_id": decision.id,
                        "basis": ("signal promotion requires owning-team lead "
                                  "or coordinator fallback"),
                        "approvers": approvers}
